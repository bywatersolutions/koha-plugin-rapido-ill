    ...
    return try {
        Koha::Database->schema->storage->txn_do(
            sub {
                ...
                # Due dates are ILS-defined: the checkout due date calculated by
                # Koha's circulation rules (via add_issue/AddIssue) is authoritative.
                # Any dueDate returned by Rapido is intentionally ignored.
            }
        );
        return $self;
    } catch {
