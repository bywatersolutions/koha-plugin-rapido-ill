...
$sample_config_yaml = sprintf($sample_config_yaml, 
    $library->branchcode, 
...
subtest 'add_or_update_attributes - parameter validation' => sub {
    ...
    } qr/Missing parameter/, 'Dies when no parameters provided';
    
    throws_ok {
        $plugin->add_or_update_attributes({ request => $ill_request });
    } qr/Missing parameter/, 'Dies when attributes parameter missing';
    
    throws_ok {
        $plugin->add_or_update_attributes({ attributes => {} });
    } qr/Missing parameter/, 'Dies when request parameter missing';
};
...
subtest 'add_or_update_attributes - edge cases with special characters' => sub {
    ...
    is($stored_attrs->find({ type => 'json_field' })->value, '{"key": "value", "number": 123}', 'JSON content preserved');
    is($stored_attrs->find({ type => 'html_field' })->value, '<p>HTML content &amp; entities</p>', 'HTML content preserved');
