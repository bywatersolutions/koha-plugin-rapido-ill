...
subtest 'sync_circ_requests - parameter validation' => sub {
    ...
    } qr/Missing parameter: pod/, 'Dies when pod parameter missing';
    
    throws_ok {
        $plugin->sync_circ_requests();
    } qr/Missing parameter: pod/, 'Dies when no parameters provided';
};
...
subtest 'sync_circ_requests - return structure validation' => sub {
    ...
    $plugin_mock->mock('sync_circ_requests', sub {
        my ($self, $params) = @_;
