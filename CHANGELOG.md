...
### Fixed
- [#222] `after_circ_action` no longer skips checkins for patrons whose privacy is set to 'never', where Koha hands the hook an already anonymized checkout

## [1.10.3] - 2026-08-10

- [#218] `delete_virtual_biblio` now cancels holds and returns checkouts before deleting items, fixing `return_uncirculated` (and `force_complete`) leaving orphan records behind
...
### Added
- [#216] Dedicated `force_complete_lender` method for lending-side force complete with proper check-in, hold cancellation, status guards, and lender-specific confirmation template
...
- [#200] Print slip 500 error when notice template is missing — guard `GetPreparedLetter` return value and log warning
...
- [#198] `is_exact_duplicate` does not account for `lastUpdated`, causing repeated renewals to be skipped

## [1.8.4] - 2026-04-16

### Fixed

- [#196] `receive_unshipped` stores `barcode` instead of `itemBarcode` and `callnumber` instead of `callNumber` in ILL request attributes, breaking the `after_circ_action` checkout lookup
...
- [#173] CSV import for agencies with add/update support and auto-patron creation
...
- [#189] Task queue table on admin status page backed by REST API with server-side pagination, sorting, and filtering via kohaTable
...
- [#188] Skip borrower renewal API call when due date with buffer is in the past
- [#188] Treat Rapido "dueDate already passed" 500 responses as permanent task failure
...
- [#185] Rapido ILL administration/status page accessible from Koha's Administration home
- [#185] Status indicator dot now links to the admin status page
...
- [#181] Fix lender-side checkouts using Koha circulation rules instead of Rapido due dates with buffer

## [1.4.9] - 2026-02-25

### Added

- [#180] Add force-cancel confirmation form to borrower_cancel.inc with proper CSRF token and action buttons
- [#180] Add testing scenario for error cancelling workflow

### Changed

- [#180] Allow force-cancelling borrowing requests when Rapido reports the request cannot be cancelled through their system

## [1.4.8] - 2026-02-24

### Fixed

- [#179] Fix all RequestFailed exception handlers to use populated fields
- [#179] Fix undefined response error in get_requests.pl exception handler

## [1.4.7] - 2026-02-23

### Fixed

- [#165] Fix hold priority calculation to use proper priority instead of always setting to 1
- [#178] Remove deprecated reservefee column from bootstrap script
- [#178] Fix mock_config.json to use correct itemId instead of barcode

## [1.4.6] - 2026-02-23

### Fixed

- [#177] Restore dist directory cleanup in gulpfile build process
- [#177] Restore -e flag in sed command for cross-platform compatibility

## [1.4.5] - 2026-02-23

### Fixed

- [#176] Fix receive_unshipped workflow to correctly pass circId to Rapido API
- [#176] Fix exception handling in Backend.pm to check exception type before accessing attributes
...
- [#162] CI now tests both fresh install and upgrade path from v1.1.0 to ensure database migrations work correctly
- [#162] CI improvements: use SKIP_L10N=yes to skip translation files, rename instance to 'rapido', fetch full git history
...
### Added
- [#138] Wait for 180 seconds for KTD to startup
...
### Added
- [#134] Add QueuedTask->reset() method to reset failed tasks back to queued status
- [#134] Add reset_task.pl script to reset tasks by ID, status, action, or pod

## [1.0.19] 2025-11-19

### Fixed
- [#134] Fix HTTP::Response serialization in exceptions - extract status_code, status_message, and response_body instead of serializing the entire response object
...
### Changed
- Upgrade build tooling to Gulp 5 to address security vulnerabilities in development dependencies
...
### Fixed
- [#128] Fix receive_unshipped manual step to create holds and store attributes like item_shipped does

## [1.0.13] - 2025-11-05

### Fixed
- [#137] Fix add_virtual_record_and_item parameter names in borrower_receive_unshipped method

## [1.0.12] - 2025-11-05

### Added
- [#126] Add --only-statuses flag to get_requests.pl to show status counts
...
### Added
- [#124] Add t::lib::Mocks::Client for HTTP client testing with parameter validation
...
- [#122] OWNING_SITE_CANCEL handling in Lender ActionHandler

## [1.0.5] - 2025-10-30

### Added
- [#118] Update `print_slips` for modern Koha features
- [#121] Handle BORROWING_SITE_CANCEL gracefully on the borrowing site (no-op)

## [1.0.4] - 2025-10-27

### Added
- [#117] Command-line script `run_command.pl` for executing Rapido ILL actions on individual requests with support for both lending and borrowing workflows

## [1.0.3] - 2025-10-22

### Added
- [#116] Renewal checkout notes functionality with configurable `renewal_request_note` and `renewal_accepted_note` messages
- [#116] Automatic checkout note creation during Rapido ILL renewal workflows for improved patron communication
...
### Added
- [#106] Added config.pl script for configuration management (dump/load YAML configurations)
