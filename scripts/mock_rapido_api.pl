...
# Load or create configuration with KTD sample data
sub load_config {
    my $config = {};
    ...
    if (-f $config_file) {
        ...
        if ($@) {
            app->log->warn("Failed to load config file: $@");
        ...
        eval {
            ...
            close $fh;
            app->log->info("Created default configuration: $config_file");
        };
        if ($@) {
            app->log->warn("Could not save config file: $@");
        }
    }
...
# Create default configuration with KTD sample data
sub create_default_config {
    return {
        scenarios => {
            ...
            mixed => {
                description => "Mixed borrowing and lending",
                sequence => [
                    { endpoint => "circulation_requests", response => "mixed_requests" },
                ]
            }
        },
        responses => {
                ...
                data => [
                    {
                        ...
                        dueDateTime => DateTime->now->add(days => 23)->iso8601 . 'Z'
                    }
                ]
            },
            lending_initial => {
